...
#    $users = Get-WmiObject -ClassName Win32_UserProfile | Where-Object { $_.LocalPath -like '*\Users*' } | select @{LABEL='Username';EXPRESSION={(Get-WmiObject Win32_UserAccount -Filter "SID = '$($_.Sid)'").Caption}},LocalPath,@{LABEL='ComputerName';EXPRESSION={$_.PSComputerName}}
    ...
    $users = Get-WmiObject -ClassName Win32_UserProfile | Where-Object { $_.LocalPath -like '*\Users*' } | Select-Object @{LABEL='Username';EXPRESSION={([ADSI]"LDAP://<SID=$($_.SID)>").SamAccountName}},LocalPath,@{LABEL='ComputerName';EXPRESSION={$_.PSComputerName}},@{LABEL='DistinguishedName';EXPRESSION={([ADSI]"LDAP://<SID=$($_.SID)>").DistinguishedName}} | Where-Object { $_.Username }
...
#            echo "SAN: $($SAN)"
        ...
        if( $ISSUER ) { Write-Output $CertInfo }
